...
bool YunoRenderer::CreateSamplerPresets()
{
    ...
    // 0) Custom: 일단 기본값으로 Wrap Aniso 넣어둔다.
    //    (나중에 MaterialDesc에 sampler override가 생기면 Flush에서 교체)
    {
...
TextureHandle YunoRenderer::CreateTexture2DFromFile(const wchar_t* path)
{
    ...
    // 상대경로면 CWD 기준으로 absolute 만들고, weakly_canonical로 정규화
    // (파일이 실제로 없으면 canonical은 실패할 수 있으니 weakly_canonical 사용)
    std::error_code ec;
    ...
    {
        // 실패 시 0 반환 (TextureManager가 더미로 대체하거나, caller가 0 처리)
        // 여기서 m_texWhite 반환으로 해도 되지만, 실패를 드러내는 게 디버깅에 유리
        return 0;
    }
    ...
    td.MipLevels = 1;                 // Step 10-3에서 mipgen 확장 가능
    td.ArraySize = 1;
    td.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    td.SampleDesc.Count = 1;
    td.Usage = D3D11_USAGE_DEFAULT;
